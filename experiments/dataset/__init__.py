...
def get_dataset(cfg: ProjectConfig):
    ...
    if cfg.dataset.type == 'co3dv2':
        ...
        exclude_sequence.extend(
            LOW_QUALITY_SEQUENCE.get(dataset_cfg.category, []))
        ...
        kwargs = dict(
            ...
            pick_sequence=() if dataset_cfg.restrict_model_ids is None else
            dataset_cfg.restrict_model_ids,
        )
        ...
        for dataset in (datasets["train"], datasets["val"]):
            for key, ann in dataset.seq_annots.items():
                ...
                assert correct_point_cloud_path.is_file(
                ), correct_point_cloud_path
        ...
        if (len(datasets['train']) == 0
            ...
            datasets = DatasetMap(train=datasets[dataset_cfg.eval_split],
