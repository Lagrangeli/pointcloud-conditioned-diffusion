...
class ConditionalPointCloudDiffusionModel(PointCloudProjectionModel):
    ...
    def __init__(
        ...
        if beta_schedule == 'custom':
            scheduler_kwargs.update(
                dict(trained_betas=get_custom_betas(beta_start=beta_start,
                                                    beta_end=beta_end)))
        else:
            scheduler_kwargs.update(
                dict(beta_start=beta_start,
                     beta_end=beta_end,
        ...
        self.scheduler = self.schedulers_map[
            'ddpm']  # this can be changed for inference
    ...
    def forward_train(self,
        ...
        timestep = torch.randint(0,
                                 self.scheduler.num_train_timesteps, (B, ),
                                 device=self.device,
                                 dtype=torch.long)
    ...
    def forward_sample(
        ...
        scheduler = self.scheduler if scheduler is None else self.schedulers_map[
            scheduler]
        ...
        progress_bar = tqdm(scheduler.timesteps.to(device),
        ...
        for i, t in enumerate(progress_bar):
            ...
            # Forward
            noise_pred = self.point_cloud_model(x_t_input,
            ...
            if (return_all_outputs and (i % return_sample_every_n_steps == 0
    ...
    def forward(self, batch: FrameData, mode: str = 'train', **kwargs):
        ...
        if isinstance(
            ...
            batch = FrameData(
                **batch)  # it really makes no sense, I do not understand it
        if mode == 'train':
            return self.forward_train(pc=batch.sequence_point_cloud,
                                      camera=batch.camera,
            ...
            num_points = kwargs.pop('num_points',
                                    get_num_points(batch.sequence_point_cloud))
